...
from PyQt import styleSheet, commomProperty
try:
...
class SAMPLE_WIDGET_TEMPLATE():
    ...
    def verticalLayout_def(self, parent_self, set_contents_margins=[0, 0, 0, 0], set_spacing=0, set_object_name='',
                           widget_list=[], set_styleSheet=''''''):

        '''

        :param parent_self: specify the parent object
        :param set_contents_margins: specify the contents margins
        :param set_spacing: specify the spacing
        :param set_object_name: specify the object name
        :param widget_list: separate the widgets with comma
        :param set_styleSheet: separate the style sheet with comma
        :return:
        '''
        # CREATE VERTICAL LAYOUT
        vertical_layout = QVBoxLayout(parent_self)
        vertical_layout.setObjectName(set_object_name)
        vertical_layout.setContentsMargins(set_contents_margins[0], set_contents_margins[1], set_contents_margins[2],
                                             set_contents_margins[3])
        vertical_layout.setSpacing(set_spacing)
        for each in widget_list:
            vertical_layout.addWidget(each)

        vertical_layout.setStyleSheet(set_styleSheet)

        return vertical_layout

    def gridLayout_def(self, parent_self='', set_contents_margins=[0, 0, 0, 0], set_horizontal_space=0,
                    set_vertical_space=0, set_object_name='', set_spacing=0, widgetList={}):
        '''
        ...
        :param set_object_name:
        :param set_spacing:
        :param widgetList: set widget Example: {'widget_name':[row, column, rowSpan, columnSpan]}
        :return:
        '''
        # CREATE GRID LAYOUT
        grid_layout = QGridLayout(parent_self)
        grid_layout.setObjectName(set_object_name)
        grid_layout.setContentsMargins(set_contents_margins[0], set_contents_margins[1], set_contents_margins[2],
                                        set_contents_margins[3])
        grid_layout.setHorizontalSpacing(set_horizontal_space)
        grid_layout.setVerticalSpacing(set_vertical_space)
        grid_layout.setSpacing(set_spacing)
        for each in widgetList:
            grid_layout.addWidget(widgetList[each], widgetList[each][0], widgetList[each][1], widgetList[each][2],
                                    widgetList[each][3])

    def horizontalLayout_def(self, parent_self, set_contents_margins=[0, 0, 0, 0], set_spacing=0, set_object_name='',
                           widget_list=[], set_styleSheet=''''''):
        '''
        ...
        :param set_object_name:
        :param widget_list:
        :param set_styleSheet: widget list should be separated by comma
        :return:
        '''
        # CREATE HORIZONTAL LAYOUT
        horizontal_layout = QHBoxLayout(parent_self)
        horizontal_layout.setObjectName(set_object_name)
        horizontal_layout.setContentsMargins(set_contents_margins[0], set_contents_margins[1], set_contents_margins[2],
                                                  set_contents_margins[3])
        horizontal_layout.setSpacing(set_spacing)
        for each in widget_list:
            horizontal_layout.addWidget(each)

        horizontal_layout.setStyleSheet(set_styleSheet)
